...
class RepVGG(nn.Module):
    def __init__(self, num_blocks, num_classes = 1000, width_multiplier=None, override_groups_map=None, deploy=False, use_se=False):
        ...
        self.stage5 = nn.Sequential(
            nn.Conv2d(in_channels=int(512 * width_multiplier[3]), out_channels=4096, kernel_size=3, stride=2),
            nn.ReLU(),
            nn.Dropout2d(),
...
if __name__=='__main__':
    ...
    model_a0 = RepVGG(num_blocks=[2, 4, 14, 1], num_classes=21, width_multiplier=[0.75, 0.75, 0.75, 2.5],
                      override_groups_map=None, deploy=True)
    model_a2 = RepVGG(num_blocks=[2, 4, 14, 1], num_classes=21, width_multiplier=[1.5, 1.5, 1.5, 2.75],
                      override_groups_map=None, deploy=True)
    model_b0 = RepVGG(num_blocks=[4, 6, 16, 1], num_classes=21, width_multiplier=[1, 1, 1, 2.5],
                      override_groups_map=None, deploy=True)
    model_b2 = RepVGG(num_blocks=[4, 6, 16, 1], num_classes=21, width_multiplier=[2.5, 2.5, 2.5, 5],
                      override_groups_map=None, deploy=True)
    model_b3 = RepVGG(num_blocks=[4, 6, 16, 1], num_classes=21, width_multiplier=[3, 3, 3, 5],
                      override_groups_map=None, deploy=True)
    from fvcore.nn import FlopCountAnalysis
    #
    a0 = FlopCountAnalysis(model_a0, x)
    a2 = FlopCountAnalysis(model_a2, x)
    b0 = FlopCountAnalysis(model_b0, x)
    b2 = FlopCountAnalysis(model_b2, x)
    b3 = FlopCountAnalysis(model_b3, x)
    out = model_b0(x)
    # x = sum(p.numel() for p in model_b3.parameters() if p.requires_grad)