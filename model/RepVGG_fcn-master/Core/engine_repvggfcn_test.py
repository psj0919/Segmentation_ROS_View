...
class Trainer():
    ...
    def test(self):
        ...
        for key, val in total_avr_precision.items():
            for key2, val2 in val.items():
                path = "/storage/sjpark/vehicle_data/precision_recall_per_class_p_threshold/new_dataloader/pretrained_modify_RepVGG/256/precision/{}/{}_{}.txt".format(key, key, key2)
                np.savetxt(path, total_avr_precision[key][key2], fmt= '%f')

        #
        for key, val in total_avr_recall.items():
            for key2, val2 in val.items():
                path = "/storage/sjpark/vehicle_data/precision_recall_per_class_p_threshold/new_dataloader/pretrained_modify_RepVGG/256/recall/{}/{}_{}.txt".format(key, key, key2)
