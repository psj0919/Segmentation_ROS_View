...
class demo:
    def __init__(self, resize_size, org_size_w, org_size_h, device, weight_path, num_class, model_name):
    ...
    def load_network(self):
        if self.model_name == "Swin2-T":
            model = SwinTransformer_UperNet(img_size=224, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), num_classes=21)
        elif self.model_name == "Swin2-S":
            model = SwinTransformer_UperNet(img_size=224, embed_dim=96, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), num_classes=21)
        elif self.model_name == "Swin2-B":
            model = SwinTransformer_UperNet(img_size=224, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), num_classes=21)
        elif self.model_name == "Swin2-L":                        
            model = SwinTransformer_UperNet(img_size=224, embed_dim=192, depths=(2, 2, 18, 2), num_heads=(6, 12, 24, 48), num_classes=21)
